...
//    Autowired tells Spring to scan for objects that need to be instantiated for a class or method to run
    ...
    //this is the login endpoint, which allows users to log in by calling the login.html template with the /login route
    @GetMapping("/login")
    public String login(Model model, HttpServletRequest request) {
        //Model model remaps Model to the model variable
        if (request.getSession(false) != null) {
            return "redirect:/";
        }
        // once logged in, the user will be redirected to / (homepage)

        model.addAttribute("user", new User());
        // send a newly created user to the template as the string user, so it can be displayed within the template
        return "login";
    }
    ...
    // above: check the user is logged in, then create variable with type List<Post>, then use a for loop
    // to get all the posts and populate them into postList, then use model.addAttributes to add these details
    // to user model so this route will populate and display the homepage.
    ...
    //routes with model.addAttribute("notice", "message") are error-handling routes
    ...
    public Model setupDashboardPage(Model model, HttpServletRequest request) throws Exception {
        ...
        //assign the value of current user (SESSION_USER) to the variable sessionUser
        Integer userId = sessionUser.getId();
        //get user ID and store in variable userId
        ...
        //model.addAttribute allows passing of info to the Thymeleaf pages when that template is called
        model.addAttribute("user", sessionUser);
        //above sends current user (sessionUser) to the Thymeleaf dashboard template as a variable called user
        model.addAttribute("postList", postList);
        model.addAttribute("loggedIn", sessionUser.isLoggedIn());
